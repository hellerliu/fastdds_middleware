...
5. 支持c/c++结构体，json，protobuf消息的发送。c/c++结构体(可包含字符串，数组，map等stl数据结构)序列化简单，不用编译中间的idl接口，可方便的用于中间件内部层通信，和MCU 的通信；protobuf可用于soc内部通信，或者网关转grpc与云端通信；json可用于网关转http，mqtt，websocket与云端通信，和内部的和python语言编写的节点通信。见normal_pub.cpp，normal_sub.cpp，演示了c/c++数据类型的订阅发布；rpc通信的接口文件中IMyTestCall.h 请求回复数据也使用的是结构体。json，protobuf使用和结构体一样，中间件在内部类型萃取，并序列化和解析，使用简单方便。结构体的序列化测试见struct_serialize_test，json序列化测试见json_serialize_test。还可以扩展ros1等序列化。
